"""
Handlers de menu do entregável de upsell (sistema de blocos)
"""

from typing import Any, Dict

from core.config import settings
from database.repos import UpsellDeliverableBlockRepository, UpsellRepository
from services.conversation_state import ConversationStateManager


async def handle_deliverable_menu(user_id: int, upsell_id: int) -> Dict[str, Any]:
    """Menu do entregável (sistema de blocos)"""
    ...
    for i, block in enumerate(blocks, 1):
        ...
        buttons_row = [
            {"text": f"{i}️⃣", "callback_data": f"upsell_del_view:{block.id}"},
            {"text": "Efeitos", "callback_data": f"upsell_del_effects:{block.id}"},
            {"text": "Mídia", "callback_data": f"upsell_del_media:{block.id}"},
            {"text": "Texto/Legenda", "callback_data": f"upsell_del_text:{block.id}"},
            {"text": "❌", "callback_data": f"upsell_del_delete:{block.id}"},
        ]
        block_buttons.append(buttons_row)

    # Botão criar bloco sempre no final
    keyboard_buttons = block_buttons + [
        [{"text": "➕ Criar Bloco", "callback_data": f"upsell_del_add:{upsell_id}"}],
        [
            {
                "text": "👀 Pré-visualizar",
                "callback_data": f"upsell_del_preview:{upsell_id}",
            }
        ],
        [
    ...
    return {
        "text": f"📦 *Entrega do Upsell: {upsell.name}{value_text}*\n\n"
        f"Cada linha representa uma mensagem que será enviada na entrega.\n\n"
        f"Total de blocos: {len(blocks)}",
        "keyboard": keyboard,
    }


async def handle_add_deliverable_block(user_id: int, upsell_id: int) -> Dict[str, Any]:
    ...
    # Criar bloco vazio
    await UpsellDeliverableBlockRepository.create_block(
        upsell_id=upsell_id,
        order=next_order,
    )

    # Voltar ao menu do entregável
    return await handle_deliverable_menu(user_id, upsell_id)
...
async def handle_delete_deliverable_block(
    ...
    block = await UpsellDeliverableBlockRepository.get_block_by_id(block_id)
    if not block:
        return {"text": "❌ Bloco não encontrado.", "keyboard": None}

    upsell_id = block.upsell_id
    await UpsellDeliverableBlockRepository.delete_block(block_id)

    return await handle_deliverable_menu(user_id, upsell_id)


async def handle_deliverable_text_click(user_id: int, block_id: int) -> Dict[str, Any]:
...
async def handle_deliverable_text_input(
    user_id: int, block_id: int, upsell_id: int, text: str
) -> Dict[str, Any]:
    """Salva texto do bloco"""
    await UpsellDeliverableBlockRepository.update_block(block_id, text=text)
    ConversationStateManager.clear_state(user_id)

    return await handle_deliverable_menu(user_id, upsell_id)


async def handle_deliverable_media_click(user_id: int, block_id: int) -> Dict[str, Any]:
    ...
    return {
        ...
        f"A mídia será reutilizada sempre que o entregável for enviado.",
        "keyboard": None,
    }


async def handle_deliverable_media_input(
    user_id: int,
    block_id: int,
    upsell_id: int,
    media_file_id: str,
    ...
    await UpsellDeliverableBlockRepository.update_block(
...
async def handle_deliverable_effects_click(
    user_id: int, block_id: int
) -> Dict[str, Any]:
    ...
    keyboard = {
        "inline_keyboard": [
            [
                {"text": "⏰ Delay", "callback_data": f"upsell_del_delay:{block_id}"},
                {
                    "text": "🗑️ Auto-deletar",
                    "callback_data": f"upsell_del_autodel:{block_id}",
                },
            ],
            [
                {
                    "text": "🔙 Voltar",
                    "callback_data": f"upsell_deliverable:{block.upsell_id}",
                }
            ],
        ]
    }
...
async def handle_deliverable_delay_click(user_id: int, block_id: int) -> Dict[str, Any]:
...
async def handle_deliverable_delay_input(
    ...
    return await handle_deliverable_effects_click(user_id, block_id)


async def handle_deliverable_autodel_click(
    user_id: int, block_id: int
) -> Dict[str, Any]:
...
async def handle_deliverable_autodel_input(
...
async def handle_preview_deliverable(user_id: int, upsell_id: int) -> Dict[str, Any]:
    ...
    from services.upsell import DeliverableSender
    ...
    # Enviar o entregável completo ao usuário (preview, sem bot_id para não usar stream)
    ...
    api = TelegramAPI()
    menu_data = await handle_deliverable_menu(user_id, upsell_id)
